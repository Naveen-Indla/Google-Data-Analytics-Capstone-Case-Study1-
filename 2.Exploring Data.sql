...
-- NO OF ROWS 
select count(*) from `CASE_STUDY1`.`COMBINED_DATA`;
...
-- CALCULLATE USAGE TIME OF BIKE MORE THAN A DAY 
...
from `CASE_STUDY1`.`COMBINED_DATA`; -- return = 1891

select count(distinct start_station_id) as number_unique_station_id
from `CASE_STUDY1`.`COMBINED_DATA`; -- return = 3360
...
from `CASE_STUDY1`.`COMBINED_DATA`; -- return = 1898

select count(distinct end_station_id) as number_end_unique_station_id
from `CASE_STUDY1`.`COMBINED_DATA`; -- return = 3382
...
count(end_station_name),count(end_station_id),count(end_lat),count(end_lng)
from `CASE_STUDY1`.`COMBINED_DATA`;
...
-- CHECK member_casual - 2 UNIQUE VLAUES
